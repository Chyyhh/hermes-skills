"""
A500 Daily Data Downloader — Tencent Finance API (HTTP/2)
Fast, reliable, with retry + resume.

Usage:
  python3 download_a500.py              # download latest incomplete date
  python3 download_a500.py 20260609     # specific date
  python3 download_a500.py --all        # fill all missing dates

API: web.ifzq.gtimg.cn (Tencent Finance)
Returns: [date, open, close, high, low, volume] where volume is in 手 (×100)
Speed: ~80s for 250 stocks, 97%+ success rate
"""
...
if __name__ == '__main__':
    ...
    if '--all' in sys.argv:
        ...
        conn.close()
        latest = counts[0][0] if counts else 'N/A'
        for d, c in counts:
            if c < 400:
